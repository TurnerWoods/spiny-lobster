      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Card variant="glass">
          ...
          <CardContent className="space-y-3 text-sm">
            <div>
              <span className="text-foreground/75 block mb-1">Primary Goal</span>
            ...
            {healthGoals.secondaryGoals.length > 0 && (
              <div>
                <span className="text-foreground/75 block mb-1.5">Additional Goals</span>
            ...
            {healthGoals.targetTimeline && (
              <div>
                <span className="text-foreground/75">Timeline: </span>
        ...
        <Card variant="glass">
          <CardHeader className="pb-3">
            <CardTitle className="flex items-center justify-between text-base text-rich-black">
              ...
              {medicalFieldsCount > 0 ? (
                <span className="text-xs text-foreground/70">{medicalFieldsCount} fields</span>
              ) : (
          ...
          <CardContent className="space-y-2 text-sm">
            {medicalHistory.currentWeight || medicalHistory.targetWeight ? (
              <>
                <div className="flex justify-between">
                  <span className="text-foreground/75">Weight:</span>
            ...
            {(medicalHistory.heightFeet || medicalHistory.heightInches) && (
              <div className="flex justify-between">
                <span className="text-foreground/75">Height:</span>
                <span className="text-rich-black">
                  {medicalHistory.heightFeet && medicalHistory.heightInches
                    ? `${medicalHistory.heightFeet}'${medicalHistory.heightInches}"`
            ...
            {medicalHistory.medicalConditions.length > 0 && (
              <div className="pt-1">
                <span className="text-foreground/75 block mb-1">Conditions</span>
            ...
            {medicalHistory.currentMedications && (
              <div className="pt-1">
                <span className="text-foreground/75 block mb-1">Medications</span>
            ...
            {medicalFieldsCount === 0 && (
              <p className="text-foreground/70 italic">No medical information provided</p>
            )}
          </CardContent>
        </Card>
          ...
          <CardHeader className="pb-3">
            <CardTitle className="flex items-center justify-between text-base text-rich-black">
              ...
              {lifestyleFieldsCount > 0 ? (
                <span className="text-xs text-foreground/70">{lifestyleFieldsCount} fields</span>
              ) : (
                <span className="text-xs text-foreground/70">Optional</span>
              )}
            </CardTitle>
            ...
            {lifestyleFieldsCount > 0 ? (
              <div className="grid gap-3 text-sm sm:grid-cols-3">
                {lifestyle.exerciseFrequency && (
                  <div>
                    <span className="text-foreground/75 block mb-0.5">Exercise</span>
                ...
                {lifestyle.stressLevel && (
                  <div>
                    <span className="text-foreground/75 block mb-0.5">Stress Level</span>
                    <span className="text-rich-black">{stressLabels[lifestyle.stressLevel]}</span>
                  </div>
                )}
                {lifestyle.sleepHours && (
                  <div>
                    <span className="text-foreground/75 block mb-0.5">Sleep</span>
                    <span className="text-rich-black">{sleepLabels[lifestyle.sleepHours]}</span>
                  </div>
                )}
              </div>
            ) : (
              <p className="text-sm text-foreground/70 italic">No lifestyle information provided</p>
            )}
          </CardContent>
        </Card>
      </div>
        ...
        <Card className={`border-2 transition-colors ${agreedToTerms ? "border-accent-gold/40 bg-accent-gold/5" : "border-warm-stone/20 bg-warm-stone/5"}`}>
          <CardContent className="flex items-start gap-4 pt-6">
            ...
            <div className="space-y-4 flex-1">
              ...
              {!agreedToTerms && (
                <p className="text-xs text-foreground/75 flex items-center gap-1">
